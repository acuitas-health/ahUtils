...
report_config <- function(config = NULL, report_todo = FALSE) {
  ...
  rc <- tibble::tibble(Config = character(), Details = character())
  if (!report_todo) {
    ...
    if (!is.null(config$report_name)) {
      if (config$report_name != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Report Name:", Details = config$report_name)
      }
    }
    if (!is.null(config$report_author)) {
      if (config$report_author != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Report Author:", Details = config$report_author)
      }
    }
    if (!is.null(config$requested_by)) {
      if (config$requested_by != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Requested By:", Details = config$requested_by)
      }
    }
    if (!is.null(config$wrike_task)) {
      if (config$wrike_task != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Wrike Task:", Details = config$wrike_task)
      }
    }
    if (!is.null(config$gitlab_repo)) {
      if (config$gitlab_repo != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Gitlab Repo:", Details = config$gitlab_repo)
      }
    }
    if (!is.null(config$application)) {
      if (config$application != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Application Name:", Details = config$application)
      }
    }
    if (!is.null(config$strat_col)) {
      if (config$strat_col != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Stratified By:", Details = config$strat_col)
      }
    }
    if (!is.null(config$dsn_name)) {
      if (config$dsn_name != "TODO") {
        rc <-
          rc |>
          tibble::add_row(Config = "Data Source:", Details = config$dsn_name)
      }
    }
  } else {
